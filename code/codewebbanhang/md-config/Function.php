<?php 
    ...
    **/
    ...
    function base_url()
    {
        ...
        return $url  = "http://localhost:8080/codewebbanhang/"; //tạo link url nhanh 
    }
...
function rutgonduc_text($text) {
    
    $duc_text = strip_tags($text);
    $blah = explode(' ', $duc_text);

    if (count($blah) > 35) {
        $k = 35;//luong text muon  hien thi
        $use_dotdotdot = 1;
    } else {
        $k = count($blah);
        $use_dotdotdot = 0;
